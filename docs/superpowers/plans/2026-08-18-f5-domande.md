...
## Lezione trovata durante l'esecuzione
...
**`on conflict do nothing` non ha alcun effetto** sulle insert di fixture di
questa tabella: non esiste un vincolo di unicità su `(category, body)` che lo
renda un'operazione idempotente, quindi ogni `beforeEach` che la usa aggiunge
righe nuove invece di riusare le stesse. Nel Task 2 questo ha fatto fallire il
test sul riciclo della categoria esaurita (che richiede un conteggio esatto di
domande in una categoria). Corretto lì con una `delete` esplicita per body
prima dell'insert. **Applica lo stesso pattern in ogni test che inserisce
fixture in `questions`**, anche dove il conteggio non sembra contare: righe
che si accumulano silenziosamente da un test all'altro sono un problema anche
quando non fanno fallire nulla oggi.
